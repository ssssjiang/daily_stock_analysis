...
class Config:
    ...
    # Watched sector names for market review (fuzzy match against API sector names)
    market_review_watched_sectors: List[str] = field(default_factory=list)
    ...
    def _load_from_env(cls) -> 'Config':
        ...
        return cls(
            ...
            market_review_watched_sectors=[
                s.strip() for s in os.getenv('MARKET_REVIEW_WATCHED_SECTORS', '').split(',') if s.strip()
            ],
