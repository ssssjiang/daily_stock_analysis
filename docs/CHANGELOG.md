...
### 新增（#minor）
- **大盘复盘：关注板块追踪**
  - 新增配置项 `MARKET_REVIEW_WATCHED_SECTORS`（逗号分隔，模糊匹配板块名）
  - 复盘报告新增「### 五、关注板块追踪」章节，展示各关注板块当日涨跌幅与全市场排名
  - LLM 对每个关注板块生成专项简析（1-2 句）
  - 搜索服务自动为每个板块检索当日资讯（最多 3 条），以 Markdown 链接形式注入报告
  - 实现细节：`get_all_sectors()` 单次 API 调用同时派生 top5/bottom5/关注板块；链接由代码注入而非 LLM 生成，避免幻觉
  - 回滚方式：不配置 `MARKET_REVIEW_WATCHED_SECTORS` 则完全跳过，报告结构与原版相同
